...
function analyzeSymptoms(input) {
  ...
  const conditions = scored.map((s, i) => ({
    name: s.def.name,
  ...
  if (conditions.length === 0) {
    conditions.push({
      ...
      confidence: 30,
      explanation: "The description did not match a clear pattern. Adding detail about location, duration and severity can improve the insight.",
  ...
  matched.forEach(
    (s) => categoryMap.set(s.category, (categoryMap.get(s.category) ?? 0) + s.weight)
  );
